...
class ConfigLoader:
    ...
    def load_with_defaults(self, defaults_path: str, override_path: str, convert_to_radians: bool = False) -> dict:
        """
        ...
        Args:
            ...
            convert_to_radians: If True, convert joint angles from degrees to radians
        ...
        if convert_to_radians:
            config = self._convert_joints_to_radians(config)
        
        return config
    
    def _convert_joints_to_radians(self, config: dict) -> dict:
        """
        Convert joint angles from degrees to radians
        Dynamically finds and converts all numeric lists in joints structure
        No hardcoded keys - works with current YAML structure only (no legacy fallback)
        """
        result = copy.deepcopy(config)
        
        if 'joints' not in result:
            return result
        
        joints = result['joints']
        
        # Recursively convert all numeric lists in joints structure
        # Only processes current structure - no legacy fallback support
        self._convert_numeric_lists_to_radians(joints)
        
        return result
    
    def _convert_numeric_lists_to_radians(self, obj: Any) -> None:
        """
        Recursively convert numeric lists (degrees) to radians
        Works with any nested structure - no hardcoded keys, no legacy fallback
        """
        if isinstance(obj, dict):
            for key, value in obj.items():
                if isinstance(value, list) and len(value) > 0:
                    # Check if it's a list of numbers (degrees)
                    if all(isinstance(x, (int, float)) for x in value):
                        # Convert to radians
                        obj[key] = [math.radians(deg) for deg in value]
                    else:
                        # Recursively process nested structures
                        self._convert_numeric_lists_to_radians(value)
            ...
            for item in obj:
                if isinstance(item, (dict, list)):
                    self._convert_numeric_lists_to_radians(item)
